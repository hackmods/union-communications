...
import { createNominationSchema } from "@/lib/validation/elections";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const parsed = parseJsonBody(createNominationSchema, raw);
  if (!parsed.ok) {
    return NextResponse.json(
  ...
  const cycle = await electionsStore.addNomination(id, parsed.data);
  await auditLog.log({
    userId: authResult.session.user.id,
    action: "elections.nomination.create",
