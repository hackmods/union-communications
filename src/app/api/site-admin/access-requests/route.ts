...
async function gate() {
  return requireSiteAdminSession();
}
  ...
  const items = await withRlsContext(
    {
      ...
      unionId: a.session.user.unionId,
      mfaVerified: true,
      crossLocal: true,
    },
    () =>
      accessRequestStore.list({
        status: ACCESS_REQUEST_STATUSES.includes(status as never)
          ? (status as (typeof ACCESS_REQUEST_STATUSES)[number])
          : undefined,
        kind: ACCESS_REQUEST_KINDS.includes(kind as never)
          ? (kind as (typeof ACCESS_REQUEST_KINDS)[number])
          : undefined,
      }),
  );
  await auditLog.log({
    userId: a.session.user.id,
    action: "site_admin.access_request.list",
...
export async function PATCH(req: Request) {
  ...
  const row = await withRlsContext(
    ...
    () =>
      accessRequestStore.update(id, {
        ...parsed.data,
        reviewedById: a.session.user.id,
      }),
  );
  if (!row) return NextResponse.json({ error: "Not found" }, { status: 404 });
  await auditLog.log({
    userId: a.session.user.id,
    action: "site_admin.access_request.update",
    resourceType: "access_request",
    resourceId: id,
