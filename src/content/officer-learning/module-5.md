# Module 5: Financial Health, Double-Signature Controls & Auditing Compliance
...
*   **The Control Standard**:
    *   **Every check or electronic bank transfer must be signed/authorized by exactly two of the designated signing officers**.
    *   *No signing officer can ever sign a check made out to themselves* (e.g., for expense reimbursement).
    *   *Under no circumstances should any officer sign a blank check*.
...
1.  **Bank Reconciliation**: Matching the bank statement balance with the Treasurer's general ledger balance.
2.  **Outstanding Cheques**: Identifying checks that were written but have not yet been cashed by the recipient.
3.  **Voucher Verification**: Selecting random ledger transactions and verifying that an original receipt and a double-signed voucher exist for each one.
4.  **Dues Reconciliation**: Verifying that the dues rebated from the national union match the membership rosters.
...
⚠️ Warning: Single-signer e-transfer apps on one officer's phone bypass every control this module describes. Configure the bank so two approvals are required for disbursements.
...
- [ ] Every disbursement has itemized receipt + completed voucher
- [ ] Two authorized signers review each payment — no self-signing reimbursements
- [ ] No blank cheques pre-signed — ever
- [ ] Budget approved at GMM with quorum; amendments recorded
- [ ] Monthly or quarterly variance reports to executive
- [ ] Trustees elected separately from executive; no dual office
- [ ] Six-month audit: bank rec, outstanding cheques, random voucher sample, dues match
- [ ] Audit report presented to membership with corrective motions as needed
- [ ] Delegate and conference claims tied to GMM or executive authorization motion
- [ ] Lost-wage reimbursement includes proof of missed scheduled shifts
- [ ] E-transfer and electronic payments require two-officer authorization with audit log
- [ ] Arbitration and legal invoices paid from voted reserve line when available
- [ ] Variance report flags any line over 10% of plan with written explanation
- [ ] No "miscellaneous" spending without sub-lines members can question at GMM

---

## Common pitfalls

1. **Total-only credit slips** — Prove trade union purpose and line items, not just amount.
2. **Vacation blank cheques** — "Emergency" pre-signing destroys double control.
3. **Trustee on executive** — Dual roles undermine audit credibility.
4. **Budget ghost lines** — "Miscellaneous" hides spending; members cannot democratically prioritize.
5. **Skipped six-month audits** — Small errors compound; trust erodes before fraud even appears.
...
### Question 1
The Local President submits an expense claim for $150 to reimburse them for buying lunch for the executive board after a bargaining session. They provide a credit card slip showing a total of $150 but no itemized receipt showing what was ordered. Can the Treasurer issue a reimbursement check?
*   A) Yes, because the President is a trusted officer and the amount is under $200.
*   B) No. All expense claims must be supported by original, itemized receipts showing exactly what was purchased. Total-only credit card slips are not sufficient.
*   C) Yes, but only if the Vice-President signs the check instead of the Treasurer.
*   D) No, unless the President agrees to pay back the tax portion out of pocket.

**Correct Answer: B**
*Explanation*: To ensure financial compliance and prevent fraud, all union expenditures must be backed by original, itemized receipts. Credit card total slips do not prove that the expenditure was solely for legitimate trade union purposes (e.g., they don't show if alcohol or personal items were purchased).
