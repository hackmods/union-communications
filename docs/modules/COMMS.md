...
Home landing (`HomeContent`): **v1** ships a Brand Kit primary CTA + “What’s next” roadmap secondary, dual-path cards (comms vs Officer Hub, or compact “coming soon” when hub is off), Brand Kit link row, and a four-column toolbox (boards → print → social → website). **Composition v2 is open** when the hero fails the empty-half / unfinished-mock test — agents follow [`.cursor/rules/public-marketing-ux.mdc`](../../.cursor/rules/public-marketing-ux.mdc) and [`.cursor/rules/responsive-layouts.mdc`](../../.cursor/rules/responsive-layouts.mdc) (two-zone hero at `lg+` inside `PAGE_SHELL.wide`; shell alignment ≠ sparse left column). Pair any redesign with `COPY-001` CTA de-dup + smoke updates. First week (`/guide/social-media-plan`) is the channel roadmap **after** Brand Kit. Optional fifth-channel email guide at `/guide/email-broadcast`. Operators verify deploys with `GET /api/health` (`npm run health:check`). Deferred content ideas: [`COMMS_BACKLOG.md`](COMMS_BACKLOG.md). Agent nav conventions: `.cursor/rules/comms-public-nav.mdc`.
