...
### [SEC-003] ✅ CLOSED (2026-07-23)
**Category:** Security
**Severity/Priority:** Critical
**Status:** Closed — Drizzle schemas + migrations through `0009` (RLS + `0008_app_role` non-owner `unionops_app`), docker Postgres + init password sync, memory-data Hub banner, grievance/bumping/audit/time/attachments/discussions(/tasks) Drizzle adapters behind `*_DB_BACKEND` flags (defaults remain **memory** for safe demo compose), idempotent `npm run db:seed`, RLS contract unit test + `npm run db:rls-smoke`, `npm run db:durability-smoke`. Operators flip backends + app-role `DATABASE_URL` per host; production default flip is an ops choice, not a code gap.
...
### [FEAT-001] ✅ CLOSED (2026-07-23)
**Category:** Feature Parity
**Severity/Priority:** High
**Status:** Closed — durable local + S3-compatible storage, ClamAV HTTP client, SSE-S3 AES256; CMEK remains optional/stretch.
**Shipped:** Drizzle `attachment_meta` + `documents` tables (migration `0006_attachments`); `ATTACHMENT_STORAGE=local` filesystem object store; `ATTACHMENT_STORAGE=s3` via `@aws-sdk/client-s3` (MinIO/R2/AWS env: endpoint, region, bucket, keys, `FORCE_PATH_STYLE`, `ATTACHMENT_S3_SSE=AES256` on PutObject); async `scanAttachment` POSTs to `${ATTACHMENT_SCANNER_URL}/scan` (octet-stream; JSON `{ok,infected?}` or ClamAV `OK`/`FOUND` text); fail-closed on scanner errors unless `ATTACHMENT_SCAN_ALLOW_SKIP_ON_ERROR`; Local Documents vault; optional compose `clamav` profile (daemon stub — REST proxy still operator-owned); COMPLIANCE/SETUP encryption notes; unit tests with mocked S3 + fetch.
**Still open (stretch):** CMEK / SSE-KMS customer-managed keys; signed PUT/GET URL upload flow (bytes still go through app APIs today).
...
**Implementation Blueprint:**
1. Add S3-compatible object storage integration (MinIO for self-host, S3/R2 for cloud) behind the existing `AttachmentMeta.storageKey` field — swap the memory adapter for a real upload/download-URL flow (signed PUT/GET URLs). ✅ (direct Put/Get via SDK; signed URLs optional stretch)
...
4. Add encryption-at-rest for the object storage bucket/volume (SSE-S3 minimum, customer-managed keys as a stretch goal) given the "highly confidential" classification in `docs/COMPLIANCE.md`. ✅ SSE-S3; CMEK stretch
...
**Status:** Closed 2026-08-12 — Home composition v2 two-zone hero + path uses `pathCommsCta` only; smoke asserts preview + de-dup when hub public
**Problem/Gap Statement:** When the Officer Hub is public, `HomeContent` rendered the same **Brand Kit** + **Open the first-week roadmap** button pair twice — once in the hero and again in the Comms path card. The `lg+` hero also failed the empty-half test (full-bleed gradient + left-only mark/copy).
**Resolution:** `HomeHeroPreview` secondary zone; Comms path primary is `home.pathCommsCta` ("Get started" / "Commencer"); roadmap outline stays hero-only. Rules: [`public-marketing-ux.mdc`](../.cursor/rules/public-marketing-ux.mdc).
**Affected Architecture/Files:**
- `src/components/pages/HomeContent.tsx`, `HomeHeroPreview.tsx`
- `messages/en.json`, `messages/fr.json` (`home.heroPreview*`, `home.pathCommsCta`)
- `e2e/builders.smoke.spec.ts`
**Acceptance Criteria:**
- No duplicate accessible brand-setup link name in hero + path.
- At ~1280 / ~1536, hero has two zones (message + product preview).
...
1. ~~`FUTURE-002` DFR guide, `FUTURE-003` seniority playbook, `FUTURE-004` right-to-refuse guide~~ — **CLOSED 2026-07-23**.
2. ~~`FUTURE-001` Steward Quick-Log~~ — **CLOSED 2026-07-23**.
3. `FUTURE-005` Action Card / QR petition builder — ✅ CLOSED (MVP); full in-product petition collection still requires a separate ADR + compliance pass.
...
### [FUTURE-001] ✅ CLOSED (2026-07-23)
**Category:** Feature Parity
**Severity/Priority:** Medium (high value, low architectural risk)
**Status:** Closed — `InformalLogEntry` (not overloading `MemberCommunication`); Hub module `"informalLog"`; memory + optional `INFORMAL_LOG_DB_BACKEND=postgres` (`0010_informal_log` + RLS); `/app/informal-log` + `/api/informal-log` (`canManageQolContent`); convert-to-grievance stamps `convertedToGrievanceId` + seeds note/communication.
...
**Affected Architecture/Files:** `src/types/informal-log.ts`, `src/lib/informal-log/*`, `src/app/api/informal-log/**`, `src/app/[locale]/app/informal-log/page.tsx`
**Implementation Blueprint:**
...
### [FUTURE-002] ✅ CLOSED (2026-07-23)
**Category:** Feature Parity (content)
**Severity/Priority:** Low (effort) / High (trust value)
**Status:** Closed — `/guide/dfr` (`dfrGuide.*` EN/FR), GuideLayout + SourcesBlock + not-legal-advice Callout; linked from Blueprint related paths + labour guides strip; sitemap.
...
### [FUTURE-005] ✅ CLOSED (2026-07-23)
**Category:** Comms Tools
**Severity/Priority:** Low–Medium, **scope-gate required before starting**
**Status:** Closed — MVP only: `/tools/action-card` (headline / ask / deadline / CTA, Brand Kit theme, QR → officer-supplied external petition URL, PNG/PDF). No in-product signature collection. Full petition/pledge ADR remains out of scope (see blueprint §3).
...
### [FUTURE-006] ✅ CLOSED (2026-07-23)
**Category:** Feature Parity
**Severity/Priority:** Medium
**Status:** Closed — authoring (`/tools/pulse-poll`) + response collection: `PollDefinition`/`PollResponse`; memory + optional `POLLS_DB_BACKEND=postgres` (migration `0017_polls` + RLS); public `/poll/[slug]` + `POST /api/polls/[slug]/responses` (consent, IP hash rate-limit only); Hub `/app/polls` aggregates + CSV/XLSX; Publish from Pulse Poll; ADR-015. **No petition signatures.**
...
**Affected Architecture/Files:** `src/types/polls.ts`, `src/lib/polls/*`, `src/lib/auth/polls-session.ts`, `src/app/api/polls/**`, `src/app/[locale]/poll/[slug]/page.tsx`, `src/app/[locale]/app/polls/page.tsx`, `src/components/hub/PollsBoard.tsx`, `src/app/[locale]/tools/pulse-poll/**`, `docs/DECISIONS.md` ADR-015
**Implementation Blueprint:**
...
- **(A) Genuinely missing, in-scope, worth building** — `ORG-001` to `ORG-006` and `ORG-008` below.
...
### [ORG-008] ✅ CLOSED (2026-07-23)
**Category:** Feature Parity
**Severity/Priority:** Medium
**Status:** Closed — travel auth + cash advance + expense claim + ledger post on advance/reconcile; memory + optional `TRAVEL_DB_BACKEND=postgres` (`0016_travel` + RLS); `/api/travel/**` RBAC; Hub `/app/travel`; PDF/XLSX export; receipt ZIP via `buildReceiptZip` (claim attachments from object storage under `receipts/` + report buffers); `AttachmentMeta.expenseClaimId` + `listForExpenseClaim`. No SAP/ERP integration.
...
**Affected Architecture/Files:** `src/types/travel.ts`, `src/lib/travel/*`, `src/lib/auth/travel-session.ts`, `src/app/api/travel/**`, `src/app/[locale]/app/travel/page.tsx`, `src/components/hub/TravelBoard.tsx`, `src/lib/db/migrations/0016_travel.sql`, `AttachmentMeta.expenseClaimId`, `listForExpenseClaim`
**Implementation Blueprint:**
...
**Affected Architecture/Files:** src/lib/observability/**, src/instrumentation*.ts, src/sentry.*.config.ts, 
ext.config.ts, src/proxy.ts, src/lib/ops/health-status.ts, .github/workflows/ci.yml, docker/Dockerfile
**Implementation Blueprint:**
