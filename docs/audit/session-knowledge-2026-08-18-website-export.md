...
1. **WordPress classic theme ZIP shipped (Phase 1).** Second serializer wrapping today’s HTML/CSS/JS. GitHub Pages ZIP stays the default. Copy states UnionOps does **not** support WordPress. WXR-only is still weaker (no styles; media needs hosted URLs). Full Site Editing / `theme.json` is **not** shipped.
