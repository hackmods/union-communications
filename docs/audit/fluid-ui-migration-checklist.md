...
**Out of scope (intentionally not migrated):** Officer Learning ModuleViewer (platform light `olTheme`, not GuideLayout); `/privacy` `/security` `/accessibility`; canvas/`ToolEditorLayout` interiors and tool form Cards; PDF export; Hub `/app/**` + Portal `/portal/**` (deferred product decision). Website guide `PartFrame` remains an accepted exception per guide-layout-standards.
