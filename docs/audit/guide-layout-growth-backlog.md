...
| **Public workspace forms** | `/onboarding` + Brand Kit: `TOOL_COMPOSITION.editor` (`wide` + `PublicHubPanel`); `/install` `/support` `/manifesto` hub `wide` | Do not park Brand Kit–class wizards or multi-column promise/CTA grids on `focus`. Feedback / RSVP tokens stay `focus`. |
