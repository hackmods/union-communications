...
| **Legal pamphlets** | Hub `wide` + `PublicHubPanel` / callout grids (`privacy` / `security` / `accessibility`, 2026-09-20) | Keep guide-ui primitives; do not re-narrow to `GuideLayout preset="narrow"`. Feedback/RSVP tokens stay form-focused when content is a single form only. |
...
| **Public workspace forms** | `/onboarding` + Brand Kit: `TOOL_COMPOSITION.editor` (`wide` + `PublicHubPanel`); `/install` `/support` `/manifesto` `/updates` `/feedback` `/build` hub `wide` | Do not park Brand Kit–class wizards or multi-panel trust pages on `focus`/`narrow`. |
