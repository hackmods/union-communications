...
| **Public workspace forms** | `/onboarding` + Brand Kit: `TOOL_COMPOSITION.editor` (`wide` + `PublicHubPanel`); `/install` `/support` hub `wide` | Do not park Brand Kit–class wizards on `focus`. Manifesto / feedback / RSVP tokens stay `focus`. |
